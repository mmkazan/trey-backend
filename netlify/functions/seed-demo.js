...
exports.handler = async (event) => {
  ...
  // Reviews: 2 waiting, 2 replied.
  const reviewsStore = blobsStore("reviews");
  const reviews = [
    { id: "demo-1", name: "Sarah J", rating: 5, comment: "Brilliant service, car was ready early!", status: "pending", reply: "Hi Sarah, thanks so much — really glad we got you sorted quickly. See you next time! — Mik" },
    { id: "demo-2", name: "Tom H", rating: 4, comment: "Great value and a quick turnaround.", status: "pending", reply: "Thanks Tom — appreciate you taking the time, and glad it was a quick one. — Mik" },
    { id: "demo-3", name: "Priya K", rating: 5, comment: "Lovely team, really helpful and honest.", status: "approved", reply: "Thank you Priya! Being honest with folks matters to us — see you again soon. — Mik" },
    { id: "demo-4", name: "James O", rating: 5, comment: "Fair price and did exactly what they said.", status: "approved", reply: "Cheers James — that's exactly how we like to work. Thanks for the review! — Mik" },
  ];
  let i = 0;
  for (const r of reviews) {
    const rec = {
      reviewId: r.id, locationId: loc, businessName: "Mik's Cars", reviewerName: r.name,
      rating: r.rating, comment: r.comment, source: "Trey Tappy Stand 🎴", status: r.status,
